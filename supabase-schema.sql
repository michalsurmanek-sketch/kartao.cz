...
DROP POLICY IF EXISTS "Firma vidí jen sebe" ON firms;
CREATE POLICY "Firma vidí jen sebe" ON firms
  FOR ALL USING (auth.uid() = user_id);
...
DROP POLICY IF EXISTS "Vlastní transakce" ON transactions;
CREATE POLICY "Vlastní transakce" ON transactions
  FOR ALL USING (auth.uid() = user_id);

-- Ad views: jen vlastní
ALTER TABLE ad_views ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Vlastní ad views" ON ad_views;
CREATE POLICY "Vlastní ad views" ON ad_views
  FOR ALL USING (auth.uid() = user_id);
